...
if __name__ == '__main__':
    ...
    BEST_MODEL_PATH=root+'prompt_derm/results/exp/'+args.model_name
    # RUN TESTS
    print("Performing tests. Loading model at {}".format(BEST_MODEL_PATH))
    hparams = hparams_registry.default_hparams(args.model, 'SKIN')

    algorithm_class = algorithms.get_algorithm_class(args.model)

    hparams.update(json.loads('{"lr": 5e-6, "lr_classifier": 5e-4,"batch_size":26,"wd_classifier":1e-2}'))
    print('qqqq',hparams)
