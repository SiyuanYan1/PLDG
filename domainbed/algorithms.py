...
from domainbed.lib.misc import (
    random_pairs_of_minibatches, ParamDict, MovingAverage, l2_between_dicts,my_random_pairs_of_minibatches
)
from domainbed.lib.torchmisc import grad_reverse, net_dist, GradualWarmupScheduler, get_lr
from timm.models.layers import DropPath, to_2tuple, trunc_normal_
#we currently provide ERM, prompt, prompt w/mixup and prompt w/mixup and prompt genrator
#
...
ALGORITHMS = [
    ...
    'DoPrompt_equal',
    'DoPrompt_uncern',
    'DoPrompt_groupDoPrompt_group',
    'DoPrompt_group_tol',
    'DoPrompt_smooth',
    'Latent_ERM',
    'DoPrompt_group_decompose',
    'Latent_EPVT',
    'Latent_EPVT_NOG',
    'Latent_EPVT_NOMG',
    'Latent_EPVT_N0_AMG',
...
class NetDistLogger:
    def __init__(self, featurizer, classifier):
        ...
        self.featurizer_init = copy.deepcopy(featurizer)
        self.classifier_init = copy.deepcopy(classifier)
        

    def log_dist(self):
        self.global_iter += 1
        if self.global_iter % self.log_freq:
            return
        
        featurizer_dist = net_dist(self.featurizer, self.featurizer_init)
        classifier_dist = net_dist(self.classifier, self.classifier_init)
        print(f"[step {self.global_iter}] featurizer dist: {featurizer_dist:.5f}, classifier dist: {classifier_dist:.5f}")
        
        self.featurizer_init = copy.deepcopy(self.featurizer)
        self.classifier_init = copy.deepcopy(self.classifier)
...
class ERM(Algorithm):
    ...
    def get_act(self, x, domain=None):
        return self.featurizer(x)


class Latent_ERM(Algorithm):
    """
    Empirical Risk Minimization (ERM) using latent domain
    """

    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(Latent_ERM, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        self.global_iter = 0
        self.featurizer = networks.Featurizer(input_shape, self.hparams)
        self.classifier = networks.Classifier(
    ...
    def update(self, minibatches, unlabeled=None):
        all_x = torch.cat([x for x, y in minibatches])
        all_y = torch.cat([y for x, y in minibatches])
        feature=self.featurizer.get_feature_from_input(all_x)
        cls=feature[-1][:,0,:] #(50,768)  #the clas token from the last encoder block of VIT
...
class PrependPrompt_two():
    def __init__(self, network, domain_token,shared_token):
        self.featurizer = network
        self.domain_tokens = domain_token
        self.shared_tokens=shared_token
    ...
    def add_domain_prompt(self):
        ...
        def _add_domain_prompt(model, x):
            act = x[0]   #(96,197 token ,768)
            x_new = torch.cat([act, domain_tokens,self.shared_tokens], dim=1)  #(96,201,768)
            return (x_new, )
        return _add_domain_prompt

    def __enter__(self):
    #dropout紧接着 projecting, poitional token, cls token
...
class PrependPrompt_decomp():
    ...
    def add_domain_prompt(self):
        ...
        def _add_domain_prompt(model, x):
            act = x[0]  # (96,197 token ,768)
          #domain and share prompt: (128,10,768)
...
class PrependPrompt():
    def __init__(self, network, tol_tokens):
        self.featurizer = network
        self.tol_tokens = tol_tokens
        self.pos_embeddings = self.featurizer.network.encoder.pos_embedding

    def add_domain_prompt(self):
        tol_tokens=self.tol_tokens

        def _add_domain_prompt(model, x):
            act = x[0]  # (96,197 token ,768)
            x_new = torch.cat([act, tol_tokens], dim=1)  # (96,201,768)
            return (x_new,)
...
class DoPrompt(ERM):
    ...
    def x_domain_prompt_comb(self, all_bias):
        """
        :param all_bias: (96,3,4) feature map
        :return:
        """
        prompt_tokens = self.prompt_tokens
        tokens = prompt_tokens[None, ...] #(1,3,4,768)
        bias = all_bias[..., None] #(96,3,4,1)
        comb_prompt = tokens * bias #->(96,3,4 ,768)
        comb_prompt = comb_prompt.sum(dim=1) #(96,4,768) #linear combination
        return comb_prompt
    ...
    def minibatch_domain_prompt(self, minibatches):
        ...
        #for 32 batch size, generate 3 32 domain label [0,1,2]
        domain_tokens =  self.prompt_tokens[domain_labels]  #(b*5,4,768)
        return domain_tokens

    def all_bias_gt(self, disc_labels, all_bias):
        """
        :param disc_labels: 96
        :param all_bias: 96,3,4  weights
        :return:
        """
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        loss_a = F.cross_entropy(all_logit, all_y) #conventional ce loss
        # print('PPPPPPPPPPPPP',all_logit.size(),all_y.size())
        loss_w = F.binary_cross_entropy(all_bias, gt_bias)#make domain d1 's weight to be 1 for domain 1, 0 for other domains
    ...
    def predict(self, x, domain=None):
        ...
        all_logit, all_bias = self.forward_second(x, all_z)
        return all_logit


class DoPromptMixup(ERM):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super().__init__(input_shape, num_classes, num_domains, hparams)
        self.num_domains = num_domains
        # self.eval_mode = self.hparams['eval_mode']
    ...
    @torchsnooper.snoop()
    def forward_prompt(self, minibatches):
        all_xi = torch.cat([xi for (xi, yi), (xj, yj) in random_pairs_of_minibatches(minibatches)])
        all_xj = torch.cat([xj for (xi, yi), (xj, yj) in random_pairs_of_minibatches(minibatches)])
        all_yi = torch.cat([yi for (xi, yi), (xj, yj) in random_pairs_of_minibatches(minibatches)])
        all_yj = torch.cat([yj for (xi, yi), (xj, yj) in random_pairs_of_minibatches(minibatches)])
        # all_x = torch.cat([x for x, y in minibatches])
        lam = np.random.beta(0.2,0.2)

        all_x = lam * all_xi + (1 - lam) * all_xj
        loss_dp=0
        #domain prompt: (32*3,4,768)
        domain_prompts = self.minibatch_domain_prompt(minibatches)
        with PrependPrompt(self.featurizer, domain_prompts):
            all_logit = self.network(all_x)  #(96,3,224,224), (96,7)
        loss_dp += lam * F.cross_entropy(all_logit, all_yi)
        loss_dp += (1 - lam) * F.cross_entropy(all_logit, all_yj)

        return all_logit,loss_dp
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        # domain prompt learning
        all_logit,loss_dp= self.forward_prompt(minibatches)
        #(96,7) (96)
        loss_dp.backward()
...
class DoPrompt_equal(ERM):
    ...
    def x_domain_prompt_comb(self, all_bias):
        ...
        tokens = prompt_tokens[None, ...]  # (1,3,4,768)
        bias = all_bias[..., None]  # (96,3,4,1)
        bias=torch.full(bias.size(),1/5).cuda()
        # print('xxxxxxxxxxxx',bias.size())
        # print('BBBBBBBBB',bias)
        comb_prompt = tokens * bias  # ->(96,3,4 ,768)
        comb_prompt = comb_prompt.sum(dim=1)  # (96,4,768) #linear combination
        return comb_prompt
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        # gt_bias = self.all_bias_gt(disc_labels, all_bias)
        # loss_a = F.cross_entropy(all_logit, all_y)  # conventional ce loss
        # loss_w = F.binary_cross_entropy(all_bias,
        #                                 gt_bias)  # make domain d1 's weight to be 1 for domain 1, 0 for other domains
        # (loss_a + self.hparams['lambda'] * loss_w).backward()
...
class DoPrompt_group(ERM):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        self.mlp_dim = 3072
        assert self.hparams['vit_base_16'] == True
        ##shallow prompt (1,4，768)
        self.shared_prompt_Tokens = nn.Parameter(torch.zeros(1, hparams['prompt_dim'], self.featurizer.network.hidden_dim).normal_(std=0.02))
        # prompt tokens (5,4,768)
        self.prompt_tokens = nn.Parameter(
            torch.empty(num_domains, hparams['prompt_dim'], self.featurizer.network.hidden_dim).normal_(std=0.02)
        )
    ...
    def forward_prompt(self, minibatches):
        ...
        # domain prompt: (32*3,4,768)
        domain_prompts = self.minibatch_domain_prompt(minibatches)
        shared_prompts=self.shared_prompt_Tokens.expand(domain_prompts.shape[0], -1, -1)
        with PrependPrompt_two(self.featurizer, domain_prompts,shared_prompts):
            all_logit = self.network(all_x)  # (96,3,224,224), (96,7)
        loss_dp += lam * F.cross_entropy(all_logit, all_yi)
        loss_dp += (1 - lam) * F.cross_entropy(all_logit, all_yj)
...
class DoPrompt_group_tol(ERM):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        # prompt adapter
        self.project = networks.Project(self.hidden_dim, (num_domains+1) * self.prompt_dim, self.mlp_dim)
        # optimizer
        self.prompt_opt = torch.optim.AdamW(
            [self.prompt_tokens],
            lr=self.hparams["lr_prompt"],
    ...
    def x_domain_prompt_comb(self, all_bias):
        ...
        """
        prompt_tokens = self.prompt_tokens
        shared_prompt_tokens=self.shared_prompt_Tokens
        shared_tokens=shared_prompt_tokens[None,...] #(1,1,4,768)
        tokens = prompt_tokens[None, ...]  # (1,3,4,768)
        bias = all_bias[..., None]  # (96,3,4,1)
        group_tokens=torch.cat((shared_tokens,tokens),1)
        comb_prompt = group_tokens * bias  # ->(96,3,4 ,768)
        comb_prompt = comb_prompt.sum(dim=1)  # (96,4,768) #linear combination
        return comb_prompt
    ...
    def forward_second(self, all_x, all_z):
        ...
        # (96,768)
        # (96,12)->(96,3,4)
        all_bias = self.project(hint).reshape(-1, self.num_domains+1, self.prompt_dim)
        all_bias = F.softmax(all_bias, dim=1)
        total_prompts = self.x_domain_prompt_comb(all_bias)
        # (96,4,768)
        with PrependPrompt(self.featurizer, total_prompts):
            all_logit = self.network(all_x)
        return all_logit, all_bias
...
class DoPrompt_smooth(ERM):
    ...
    def update(self, minibatches, unlabeled=None,iter=None,tol_iter=None):
        self.prompt_opt.zero_grad()
        self.optimizer.zero_grad()
        self.project_opt.zero_grad()
        ...
        # smooth_alpha=0.1
        # smooth_env=(1-smooth_alpha)*gt_bias+smooth_alpha/5
        # loss_w = F.binary_cross_entropy(all_bias,
        #                                 smooth_env)  # make domain d1 's weight to be 1 for domain 1, 0 for other domains
        lambda_smooth=0.1
        # lambda_smooth=1-(5-1)/5*iter/tol_iter
        loss_w=smooth_cross_entropy_loss(all_bias,gt_bias,lambda_smooth)
        (loss_a + self.hparams['lambda'] * loss_w).backward()
...
# import torchsnooper
# @torchsnooper.snoop()
def smooth_cross_entropy_loss(pred_class_logits, gt_classes, eps=0.3, alpha=0.2, reduction='none'):
    ...
    if eps >= 0:
        ...
        # smooth_param = alpha * soft_label[torch.arange(soft_label.size(0)), gt_classes.long()].unsqueeze(1)
        smooth_param = alpha * soft_label[torch.arange(soft_label.size(0)), gt_classes.long(), :].unsqueeze(1)
    ...
    with torch.no_grad():
        ...
        targets.scatter_(1, gt_classes.data.type(torch.int64), (1 - smooth_param))
...
class DoPrompt_group_decompose(ERM):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super().__init__(input_shape, num_classes, num_domains, hparams)
        self.num_domains = num_domains
        # self.eval_mode = self.hparams['eval_mode']
        ...
        ##shallow prompt (1,4，768)
        # (10,768)
        self.shared_prompt_Tokens = nn.Parameter(
            torch.zeros(hparams['prompt_dim'], self.featurizer.network.hidden_dim).normal_(std=0.02))
        # prompt tokens (5,10)
        self.prompt_tokens_u = nn.Parameter(
            torch.empty(num_domains, hparams['prompt_dim']).normal_(std=0.02)
        )
        # prompt tokens (5,768)
        self.prompt_tokens_v = nn.Parameter(
            torch.empty(num_domains, self.featurizer.network.hidden_dim).normal_(std=0.02)
        )
        ...
        # optimizer
        self.prompt_opt = torch.optim.AdamW(
            [self.prompt_tokens_u, self.prompt_tokens_v, self.shared_prompt_Tokens],
            lr=self.hparams["lr_prompt"],
            weight_decay=1e-5
        )
        self.project_opt = torch.optim.AdamW(
    ...
    def x_domain_prompt_comb(self, all_bias, task_prompts):
        """
        :param all_bias: (96,3,4) feature map
        :return:
        """
        prompt_tokens = task_prompts
        tokens = prompt_tokens[None, ...]  # (1,3,4,768)
        bias = all_bias[..., None]  # (96,3,4,1)
        comb_prompt = tokens * bias  # ->(96,3,4 ,768)
        comb_prompt = comb_prompt.sum(dim=1)  # (96,4,768) #linear combination
    ...
    def taskprompt_decomposition(self):
        shared_prompt = self.shared_prompt_Tokens  # (10,768)
        alpha = self.prompt_tokens_u  # (5,10)
        gamma = self.prompt_tokens_v  # (5,768)
        prompt_list = []
        for k in range(self.num_domains):
            alpha_k = torch.squeeze(alpha[k, :], dim=0)  # (10,)
            gamma_k = torch.squeeze(gamma[k, :], dim=0)  # (768，）
            prompt_k = shared_prompt * torch.ger(alpha_k, gamma_k)  # (10,768)
            prompt_list.append(prompt_k)
        task_prompts = torch.stack(prompt_list, dim=0)
        # (5,10,768)
        return task_prompts

    def minibatch_domain_prompt(self, minibatches, task_prompts):
        domain_labels = torch.cat([
            torch.full((x.shape[0],), i, dtype=torch.int64, device="cuda")
            for i, (x, y) in enumerate(minibatches)
        ])
        # for 32 batch size, generate 3 32 domain label [0,1,2]
        domain_tokens = task_prompts[domain_labels]  # (b*5,4,768)
        return domain_tokens
    ...
    def forward_prompt(self, minibatches):
        ...
        loss_dp = 0
        # domain prompt: (32*3,4,768)
        task_prompts = self.taskprompt_decomposition()
        domain_prompts = self.minibatch_domain_prompt(minibatches,task_prompts)
        with PrependPrompt(self.featurizer, domain_prompts):
            all_logit = self.network(all_x)  # (96,3,224,224), (96,7)
        loss_dp += lam * F.cross_entropy(all_logit, all_yi)
        loss_dp += (1 - lam) * F.cross_entropy(all_logit, all_yj)
    ...
    def forward_second(self, all_x, all_z):
        ...
        all_bias = F.softmax(all_bias, dim=1)
        task_prompts = self.taskprompt_decomposition()
        domain_prompts = self.x_domain_prompt_comb(all_bias, task_prompts)
        # (96,4,768)
        with PrependPrompt(self.featurizer, domain_prompts):
            all_logit = self.network(all_x)
        return all_logit, all_bias
...
class Latent_EPVT(ERM):
    ...
    def forward_prompt(self, minibatches):
        ...
        all_xr=[]
        all_yr=[]
        for (xi, yi), (xj, yj), (xr, yr) in my_random_pairs_of_minibatches(minibatches):
            all_xi.append(xi)
            all_xj.append(xj)
            all_yi.append(yi)
            all_yj.append(yj)
            all_xr.append(xr)
            all_yr.append(yr)
        all_xi = torch.cat(all_xi)
        all_xj = torch.cat(all_xj)
        all_yi=torch.cat(all_yi)
        all_yj=torch.cat(all_yj)
        all_xr=torch.cat(all_xr)
        all_yr=torch.cat(all_yr)
        lam = np.random.beta(0.2, 0.2)

        all_xi=torch.cat((all_xi,all_xr),dim=0)
        all_xj = torch.cat((all_xj, all_xr), dim=0)
        all_yi=torch.cat((all_yi,all_yr),dim=0)
        all_x = lam * all_xi + (1 - lam) * all_xj
        all_yj = torch.cat((all_yj, all_yr), dim=0)
        all_x=all_x.squeeze(1)  #(120,3,224,224
        all_yi=all_yi.squeeze(1)
        all_yj=all_yj.squeeze(1)
        loss_dp = 0
        task_prompts = self.taskprompt_decomposition()
        domain_prompts = self.minibatch_domain_prompt(minibatches,task_prompts)
        with PrependPrompt(self.featurizer, domain_prompts):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        if domain_labels:
            ...
            gt_bias = self.all_bias_gt(all_pd, all_bias)
            loss_a = F.cross_entropy(all_logit, all_y)  # conventional ce loss
            loss_w = F.binary_cross_entropy(all_bias,
                                            gt_bias)  # make domain d1 's weight to be 1 for domain 1, 0 for other domains
        (loss_a + self.hparams['lambda'] * loss_w).backward()
